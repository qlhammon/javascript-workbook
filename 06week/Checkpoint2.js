...
const nums = = strNums.split(',').map(function(item) {
    return parseInt(item, 10);
});// Must split the array before turning the strings into numbers.


// Find the sum of the even values
const sumEvens = strNums.reduce((total, num) =>{
if(nums % 2==0){
  return total + num;
})// Use reduce to get a single value from the array if the nums are even.

console.log(sumEvens);

// Find the index of the first "Austin" value (the value plus its index equals 512)
const atxIdx = nums.find((item, index))=>{
  item + index == 512;
}
// We will need to use find to get the first occurence.
...
const weatherStates = weather.forEach((weather_state_name))=>{
  return weather.weather_state_name;
}
//forEach() will log a new array for just weather_state_name.
console.log(weatherStates)
//find the id of the object in weather that has a min_temp of 15.915

const idealTemp = weather.find((min_temp))=>{
  min_temp == 15.915;
  return weather.id;
}
//find() will give us the min_temp we are looking for, but make sure that we are only returning the id. 
